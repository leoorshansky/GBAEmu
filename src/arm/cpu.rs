    ...
    pub fn step(&mut self, ram: &mut Mem) {
        ...
        if let State::Arm = state {
            ...
            if self.should_execute_arm(instruction) {
                ...
                if opcode >> 4 == 3 && instruction >> 4 & 1 == 1 { // Undefined instruction
                    debug_string = "UND";
                    ...
                    if self.debug { println!("Changed mode from {:?} to Und", mode); }
                    branching = true;
                    self.regs[15] = 4;
                    self.decode_stage = NOP;
                    self.execute_stage = NOP;
                } else if (instruction << 4) >> 8 == BX_27_4 { // Branch and Exchange
                    debug_string = "BX";
                    self.regs[15] = (rm >> 1) << 1;
                    branching = true;
                    self.decode_stage = NOP;
                ...
                } else if opcode >> 2 == 2 && instruction >> 4 & 0b1111 == 0b1001 { // Atomic Swap
                ...
                } else if opcode >> 4 == 4 { // Load Multiple / Store Multiple
                    let up = instruction >> 23 & 1 == 1;
                    let pre_index = instruction >> 24 & 1 == 1;
                    ...
                    let mut memory_address = if up {
                        if pre_index { rn + 4 } else { rn }
                    } else if pre_index { rn - 4 * num_regs } else { rn - 4 * num_regs + 4 };

                    let old_rn = self.regs[rn_index];
                    if write_back {
                        self.regs[rn_index] = if up {rn + num_regs * 4} else {rn - num_regs * 4};
                    }
                    ...
                    if instruction >> 20 & 1 == 1 { // Load
                        ...
                        for bit in 0..16 {
                            if instruction >> bit & 1 == 1 {
                                let index = if load_psr && instruction >> 15 & 1 == 0 { bit as usize } 
                                    else { self.get_register_index(mode, bit) };
                                self.regs[index] = 
                        ...
                        if load_psr && instruction >> 15 & 1 == 1 {
                            ...
                            if self.debug {println!("Changed from {:?} to mode {:?}", mode, self.get_mode());}
                        }
                    } else { // Store
                        debug_string = "STM";
                        let mut first = true;
                        for bit in 0..16 {
                            if instruction >> bit & 1 == 1 {
                                let mut data = if load_psr { self.regs[bit as usize] } 
                                    else { self.regs[self.get_register_index(mode, bit)] };
                                if bit == 15 { data += 4 };
            ...
            if opcode == 3 { // Add/Sub
                ...
                let value = if immediate { value } else { self.regs[value as usize] };
                let c;
                let signed_result;
                let result = if sub {
                    debug_string = "SUB";
                    signed_result = rs as i64 - value as i64;
                    c = signed_result >= 0;
                    rs - value
                } else {
                    debug_string = "ADD";
            ...
            } else if opcode >> 2 == 0 { // Shifted Move
                let mut shifter_carry = self.get_status_bit(BIT_C);
                ...
                self.set_status_bit(BIT_N, 1 == result >> 31);
                self.set_status_bit(BIT_Z, result == 0);
            } else if opcode >> 2 == 1 { // Move / Compare // Add / Subtract Immediate
                ...
                let result = match opcode & 0b11 {
                    ...
                    1 => { // CMP
                        debug_string = "CMP";
                        signed_result = rd as i64 - immediate as i64;
                        c = signed_result >= 0;
                        write_back = false;
            ...
            } else if opcode == 8 && instruction >> 10 & 1 == 0 { // ALU
                let mut c = self.get_status_bit(BIT_C);
                let mut signed_result = 0;
                let mut write_back = true;
                let result = match instruction >> 6 & 0b1111 {
                    ...
                    5 => { // ADC
                        debug_string = "ADC";
                        signed_result = rd as i64 + rs as i64 + c as i64;
                        c = signed_result >= 1 << 32;
                    ...
                    9 => { // NEG
                        debug_string = "NEG";
                        signed_result = 0i64 - rd as i64;
                        c = signed_result >= 0;
                        0u32.wrapping_sub(rs)
                    }
                    10 => { // CMP
                        debug_string = "CMP";
                        signed_result = rd as i64 - rs as i64;
                        c = signed_result >= 0;
                        write_back = false;
                        rd.wrapping_sub(rs)
                    }
            ...
            } else if opcode == 8 { // Hi registers/ BX
                let h2 = instruction >> 6 & 1 == 1;
                let h1 = instruction >> 7 & 1 == 1;
                rd_index = if h1 { self.get_register_index(mode, (instruction & 0b111) + 8) } else { rd_index };
                let rs_index = if h2 { self.get_register_index(mode, (instruction >> 3 & 0b111) + 8) } else { rs_index };
                match instruction >> 8 & 0b11 {
                    ...
                    1 => { // CMP
                        debug_string = "CMP";
                        let signed_result = self.regs[rd_index] as i64 - self.regs[rs_index] as i64;
                ...
                if load {
                    if byte {
                        debug_string = "LDRB";
                        self.regs[rd_index] = ram.get_byte((rs + offset) as usize) as u32;
                    } else {
                        debug_string = "LDR";
                        let data = ram.get_word((rs + offset) as usize).little_endian();
                        self.regs[rd_index] = data.rotate_right(((rs + offset) % 4) * 8);
                    }
                } else if byte {
                    debug_string = "STRB";
                    ram.set_byte((rs + offset) as usize, (rd & 0xff) as u8);
                } else {
                    debug_string = "STR";
                    ram.set_word((rs + offset) as usize, Word::from_u32_le(rd));
                }
            } else if opcode >> 1 == 5 { // Load/store sign-extended byte/halfword
                let offset = self.regs[(instruction >> 6 & 0b111) as usize];
                match instruction >> 10 & 0b11 {
                    0 => {
                        debug_string = "STRH";
                        ram.set_halfword((rs + offset) as usize, HalfWord::from_u16_le((rd & 0xffff) as u16));
                    }
                    1 => {
                        debug_string = "LDSB";
                        self.regs[rd_index] = ram.get_byte((rs + offset) as usize) as i8 as u32;
                    }
                    2 => {
                        debug_string = "LDRH";
                        self.regs[rd_index] = ram.get_halfword((rs + offset) as usize).little_endian() as u32;
                    }
                    3 => {
                        debug_string = "LDSH";
                        self.regs[rd_index] = ram.get_halfword((rs + offset) as usize).little_endian() as i16 as u32;
                    }
                    _ => panic!()
                }
            } else if opcode >> 2 == 3 { // Load/Store with immediate offset
                let byte = instruction >> 12 & 1 == 1;
                let offset = (instruction >> 6 & 0b11111) << 2;
                if instruction >> 11 & 1 == 1 { // Load
                    self.regs[rd_index] = if byte {
                        debug_string = "LDRB";
                        ram.get_byte((rs + offset) as usize) as u32
                    } else {
                        debug_string = "LDR";
                        ram.get_word((rs + offset) as usize).little_endian().rotate_right(((rs + offset) % 4) * 8)
                    };
                } else { // Store
                    if byte {
                        debug_string = "STRB";
            ...
            } else if opcode >> 1 == 8 { // Load/store halfword
            ...
            } else if opcode >> 1 == 9 { // SP-relative load/store
                let offset = (instruction & 0xff) << 2;
                let sp = self.regs[self.get_register_index(mode, 13)];
                rd_index = (instruction >> 7 & 0b111) as usize;
                rd = self.regs[rd_index];
                if instruction >> 11 & 1 == 1 { // Load
                    debug_string = "LDR";
            ...
            } else if opcode >> 1 == 10 { // Load address
                let sp = instruction >> 11 & 1 == 1;
                let offset = (instruction & 0xff) << 2;
                rd_index = (instruction >> 8 & 0b111) as usize;
                debug_string = "LEA";
                self.regs[rd_index] = offset + if sp { self.regs[self.get_register_index(mode, 13)] } 
                    else { (self.regs[15] >> 2) << 2 };
            } else if opcode >> 1 == 11 && instruction >> 8 & 0b1111 == 0 { // Add offset to SP
                debug_string = "ADD_SP";
            ...
            } else if opcode >> 1 == 11 { // Push/pop registers
                ...
                if pop {
                    ...
                    for bit in 0..8 {
                        if instruction >> bit & 1 == 1 {
                            ram.set_word(memory_address as usize, Word::from_u32_le(self.regs[bit]));
                            memory_address += 4;
                        }
                    }
                    if pc_lr {
            ...
            } else if opcode >> 1 == 12 { // Load multiple/Store multiple
                let load = instruction >> 11 & 1 == 1;
                ...
                if load {
                    ...
                    if instruction >> rb_index & 1 == 0 { self.regs[rb_index] = memory_address; }
                } else {
                    debug_string = "STMIA";
                    let mut num_regs = 0;
                    let mut bits = instruction & 0xff;
            ...
            } else if opcode >> 1 == 13 && instruction >> 8 & 0b11111 == 0b11111 { // SWI
                debug_string = "SWI";
                branching = true;
                self.regs[REG_SPSR_SVC] = self.regs[REG_CPSR];
                ...
                if self.debug { println!("Changed state from Thumb to Arm"); }
                if self.debug { println!("Changed mode from {:?} to Svc", mode); }
                self.regs[15] = 8;
                self.decode_stage = NOP;
                self.execute_stage = NOP;
            } else if opcode >> 1 == 13 { // Conditional Branch
                debug_string = "B";
                ...
                self.execute_stage = NOP;
                self.regs[15] = (self.regs[15] as i32 + offset) as u32;
            } else if opcode >> 1 == 15 { // Long Branch with Link
                debug_string = "BL";
                ...
                if h { // part 2 : jump
                    ...
                    self.decode_stage = NOP;
                    self.execute_stage = NOP;
                } else { // part 1 : load address
                    self.regs[self.get_register_index(mode,14)] = (((instruction << 21) as i32) >> 9) as u32 + self.regs[15];
                }
            }
            if self.debug && executed { println!("Executed THUMB {}, rd ({}) is now {}", debug_string, rd_index, self.regs[rd_index] as i32); }
            if !branching { self.regs[15] += 2; }
        }
    }
